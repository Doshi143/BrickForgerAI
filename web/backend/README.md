...
| Image → mesh | `app/clients/mesh_gen.py` | **Working**, via `TrellisComfyUIClient` — calls a **locally-run** ComfyUI server (default `http://127.0.0.1:8188`) running a TRELLIS workflow. You run ComfyUI+TRELLIS yourself; this backend does not install or start it. |
...
2. **`brickforge_bridge.py`'s texture→vertex-color fallback is defensive,
   not the expected path** — a real TRELLIS output checked during
   development was already vertex-colored (`ColorVisuals`), so the
   `TextureVisuals` branch hasn't been exercised against a real textured
   mesh yet.
